                        ...
                        <div class="row <?php echo $menu_item['template'] == 'masonry' ? 'listing-tab-grid' : '' ?>">
                            ...
                            <?php foreach ( $menu_item['layout'] as $block ):
                                if ( isset( $block['class'] ) ) {
                                    if ( ! current_user_can('box_visible_level_1') && $block['class'] == 'level-1' ) {
                                        c27()->get_section( 'hidden-block', [
                                            'ref' => 'single-listing',
                                            'icon' => ! empty( $block['icon'] ) ? $block['icon'] : 'mi view_headline',
                                            'title' => $block['title'],
                                            'content' => '이 필드는 등업된 회원이상만 볼 수 있습니다',
                                    ...
                                    if ( ! current_user_can('box_visible_level_2') && $block['class'] == 'level-2' ) {
                                        c27()->get_section( 'hidden-block', [
                                            'ref' => 'single-listing',
                                            'icon' => ! empty( $block['icon'] ) ? $block['icon'] : 'mi view_headline',
                                            'title' => $block['title'],
                                            'content' => '이 필드는 직원이상만 볼 수 있습니다',
                                            'wrapper_class' => $block_wrapper_class,
                                            'wrapper_id' => $block['id'],
                                            'escape_html' => $escape_html,
                                    ...
                                    if ( ! current_user_can('box_visible_level_3') && $block['class'] == 'level-3' ) {
                                        c27()->get_section( 'hidden-block', [
                                            'ref' => 'single-listing',
                                            'icon' => ! empty( $block['icon'] ) ? $block['icon'] : 'mi view_headline',
                                            'title' => $block['title'],
                                            'content' => '이 필드는 관리자만 볼 수 있습니다',
                                            'wrapper_class' => $block_wrapper_class,
                                            'wrapper_id' => $block['id'],
                                            'escape_html' => $escape_html,
                                            'allow-shortcodes' => false,
                                    ...
                                    foreach ((array) $block['options'] as $option) {
                                        if ($option['name'] == 'rows') {
                                            foreach ((array) $option['value'] as $row) {
                                                if ( !$row || ! is_array( $row ) || empty( $row['show_field'] ) || ! $listing->has_field( $row['show_field'] ) ) {
                                                    continue;
                                                }
