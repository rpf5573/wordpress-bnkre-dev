                        ...
                        <div class="row <?php echo $menu_item['template'] == 'masonry' ? 'listing-tab-grid' : '' ?>">
                            ...
                            <?php foreach ( $menu_item['layout'] as $block ):
                                if ( isset( $block['class'] ) ) {
                                    // \PC::debug( ['block class' => $block['class']], __FUNCTION__ );
                                    $classes = explode( ' ', $block['class']);
                                    // \PC::debug( ['block class after explode' => $block['class']], __FUNCTION__ );
                                    if ( ! current_user_can('box_visible_level_1') && in_array('level-1', $classes) ) {
                                    ...
                                    if ( ! current_user_can('box_visible_level_2') && in_array('level-2', $classes) ) {
                                        c27()->get_section( 'hidden-block', [
                                            'ref' => 'single-listing',
                                            'icon' => ! empty( $block['icon'] ) ? $block['icon'] : 'mi view_headline',
                                            'title' => $block['title'],
                                    ...
                                    if ( ! current_user_can('box_visible_level_3') && in_array('level-3', $classes) ) {
                                        c27()->get_section( 'hidden-block', [
                                            'ref' => 'single-listing',
                                            'icon' => ! empty( $block['icon'] ) ? $block['icon'] : 'mi view_headline',
                                            'title' => $block['title'],
